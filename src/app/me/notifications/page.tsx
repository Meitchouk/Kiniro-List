...
export default function NotificationsPage() {
  ...
  const markAsRead = async (notificationId: string) => {
    ...
    try {
      ...
      setExitingIds((prev) => { const s = new Set(prev); s.delete(notificationId); return s; });
      return;
    }
  ...
  const markAllAsRead = async () => {
    ...
    await new Promise((r) => setTimeout(r, unreadIds.length * 75 + 350));
  ...
  const renderCard = (notification: UserNotification) => {
    ...
    <Card
      ...
      className={`transition-all duration-300 ease-in-out ${
        isExiting
          ? "opacity-0 scale-[0.97] translate-x-6 pointer-events-none"
          : !notification.read
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold">{t("notifications.unread")}</h2>
              {unreadCount > 0 && (
                <Button
                  variant="outline"
                  size="sm"
            ...
            {unread.length === 0 ? (
              ...
              <div className="space-y-3">{unread.map((n) => renderCard(n))}</div>
            )}
          </div>
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-lg font-semibold">{t("notifications.read")}</h2>
              {readCount > 0 && (
                <Button
                  variant="outline"
                  size="sm"
            ...
            {read.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center py-10 text-center">
                  <CheckCheck className="text-muted-foreground mb-3 h-10 w-10" />
                  <Typography variant="body2" colorScheme="secondary">
              ...
              <div className="space-y-3">{read.map((n) => renderCard(n))}</div>
            )}
          </div>
        </div>
      )}
