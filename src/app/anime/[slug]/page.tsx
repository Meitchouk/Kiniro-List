...
import { getAnimeDetail, getAnimeIdBySlug, upsertLibraryEntry, deleteLibraryEntry, getLibrary } from "@/lib/api";
...
export default function AnimeDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const { data: animeId, isLoading: isLoadingSlug, error: slugError } = useQuery({
    ...
    // Fetch anime detail
    const { data: anime, isLoading: isLoadingAnime, error: animeError, refetch } = useQuery({
        ...
        <Stack className="min-h-screen">
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <Flex className="flex-col gap-8 lg:flex-row">
                    ...
                    <Stack gap={6} className="shrink-0 lg:w-80 order-2 lg:order-1">
                        {/* Cover Image */}
                        ...
                        {/* Studios */}
                        {anime.studios?.nodes && (
                            <AnimeStudiosCard studios={anime.studios.nodes} />
                        )}

                        {/* Trailer */}
                        {anime.trailer && (
                            <AnimeTrailerCard trailer={anime.trailer} />
                        )}
                    ...
                    <Stack gap={6} className="flex-1 order-1 lg:order-2">
                        {/* Title */}
                        <div>
