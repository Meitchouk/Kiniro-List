...
import { queryLogs, clearLogs, withLogging } from "@/lib/logging";
...
export const GET = withLogging(
  async function GET(request: NextRequest) {
    try {
      ...
      // Query logs with filters (async for Firestore)
  ...
  { context: "admin:logs", errorsOnly: true }
);
...
export const DELETE = withLogging(
  async function DELETE(request: NextRequest) {
    try {
      ...
      return NextResponse.json({
        success: true,
        message: `Deleted ${result.deleted} log entries`,
